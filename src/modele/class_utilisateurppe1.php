...
//    private $selectByEmailPhoto;
        ...
        public function __construct($db)
        {
            ...
            $this->getnbtests = $db->prepare("select nbTests from utilisateurppe1 where email=:email");
            $this->updatetests = $db->prepare("update utilisateurppe1 set nbTests=:nbTests where email=:email");
            $this->selectbloquer = $db->prepare("SELECT NOW() as momentPresent, dateco as dernierEchec FROM connexions WHERE emailco =:email AND valide = 0 ORDER BY dateco DESC LIMIT 1");
        }
        ...
        public function getnbtests($email)
        {
            $this->getnbtests->execute(array(':email' => $email));
            if ($this->getnbtests->errorCode() != 0) {
                print_r($this->getnbtests->errorInfo());
            }
            return $this->getnbtests->fetch();
        }

        public function updatetests($email, $nbTests)
        {
            $r = true;
            $this->updatetests->execute(array(':email' => $email, ':nbTests' => $nbTests));
            if ($this->updatetests->errorCode() != 0) {
                print_r($this->updatetests->errorInfo());
                $r = false;
            }
            return $r;
        }

        public function selectbloquer($email)
        {
            $this->selectbloquer->execute(array(':email' => $email));
            if ($this->selectbloquer->errorCode() != 0) {
                print_r($this->selectbloquer->errorInfo());
            }
            return $this->selectbloquer->fetchAll();
        }
